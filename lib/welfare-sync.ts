...
const comparable = (policy: { id: string; title: string; category: string; region: string; amount: string; deadline: string; summary: string; target: string[]; documents: string[]; applyUrl: string; keywords: string[]; sourceName?: string | null; externalSourceId?: string | null; ministry?: string | null; department?: string | null; contact?: string | null; lifeCycles?: string[]; themes?: string[]; householdTypes?: string[]; supportCycle?: string | null; provisionType?: string | null; onlineApplyAvailable?: boolean | null; sourceRegisteredAt?: string | null; viewCount?: number | null }) => ({ id: policy.id, title: policy.title, category: policy.category, region: policy.region, amount: policy.amount, deadline: policy.deadline, summary: policy.summary, target: policy.target, documents: policy.documents, applyUrl: policy.applyUrl, keywords: policy.keywords, sourceName: policy.sourceName || '', externalSourceId: policy.externalSourceId || '', ministry: policy.ministry || '', department: policy.department || '', contact: policy.contact || '', lifeCycles: policy.lifeCycles || [], themes: policy.themes || [], householdTypes: policy.householdTypes || [], supportCycle: policy.supportCycle || '', provisionType: policy.provisionType || '', onlineApplyAvailable: policy.onlineApplyAvailable ?? null, sourceRegisteredAt: policy.sourceRegisteredAt || '', viewCount: policy.viewCount ?? null });
...
function parsePolicy(xml: string): ImportedPolicy | null {
  ...
  const department = tag(xml, ['jurOrgNm']);
  const contact = tag(xml, ['rprsCtadr']);
  const lifeCycles = tag(xml, ['lifeArray']).split(',').map((item) => item.trim()).filter(Boolean);
  const themes = theme.split(',').map((item) => item.trim()).filter(Boolean);
  const householdTypes = tag(xml, ['trgterIndvdlArray']).split(',').map((item) => item.trim()).filter(Boolean);
  const supportCycle = tag(xml, ['sprtCycNm']);
  const provisionType = tag(xml, ['srvPvsnNm']);
  const onlineValue = tag(xml, ['onapPsbltYn']);
  const detailUrl = tag(xml, ['servDtlLink', 'servUrl', 'website']);
  return { id: `bokjiro-${sourceId}`.slice(0, 190), title, category: categoryFrom(`${theme} ${targetText}`), region: '전국', amount: tag(xml, ['alwServCn', 'sprtCn', 'servDtlCn']) || provisionType || '공식 상세정보 확인', deadline: supportCycle ? `${supportCycle} 지원` : '상시 또는 공고 확인', summary: summary.slice(0, 1000), target: householdTypes.length ? householdTypes : (targetText ? [targetText] : []), documents: [], applyUrl: detailUrl || 'https://www.bokjiro.go.kr', keywords: Array.from(new Set([title, organization, ...themes, ...lifeCycles, '복지서비스'].filter(Boolean))), sourceName: organization ? `복지로 · ${organization}` : '복지로 중앙부처복지서비스 API', externalSourceId: sourceId, ministry: organization, department, contact, lifeCycles, themes, householdTypes, supportCycle, provisionType, onlineApplyAvailable: onlineValue === 'Y' ? true : onlineValue === 'N' ? false : null, sourceRegisteredAt: tag(xml, ['svcfrstRegTs']), viewCount: Number.isFinite(Number(tag(xml, ['inqNum']))) ? Number(tag(xml, ['inqNum'])) : null, isActive: false, reviewStatus: 'PENDING' };
}
